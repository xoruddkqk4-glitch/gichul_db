...
2. **[지문 검색 결과] 2x2 4분할 그리드 뷰어**
   - **[우측 상단]**: PDF에서 해당 문항 영역만 자동 크롭된 고화질 원본 인쇄 이미지 (클릭 시 새 창 확대)
   - **[우측 하단]**: TXT 형식으로 변환된 순수 영문 지문 본문 + `[지문 전체 복사]` 버튼
   - **[좌측 상단]**: HWP 파일에서 찾은 정답 번호 및 해설/해석/어휘 텍스트 + `[해설 복사]` 버튼
   - **[좌측 하단]**: 지문 메타 정보(`[고3-2024년-06월-21번]`, 발문, 일치율) + **실시간 태그 입력창 & 등록된 태그 칩(삭제 가능)**
...
### [2026-09-20 12:20] 업데이트 이력 (Commit ID: cb5577d)
- **수정 내용**:
